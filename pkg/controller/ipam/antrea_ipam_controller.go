...
func NewAntreaIPAMController(crdClient versioned.Interface,
	...
	podInformer coreinformers.PodInformer) *AntreaIPAMController {
...
func (c *AntreaIPAMController) cleanIPPoolForStatefulSet(ss *appsv1.StatefulSet) error {
	...
	allocator, err := poolallocator.NewIPPoolAllocator(poolName, c.crdClient)
	if err != nil {
...
func (c *AntreaIPAMController) Run(stopCh <-chan struct{}) {
	...
	cacheSyncs := []cache.InformerSynced{c.statefulSetListerSynced, c.namespaceListerSynced, c.podListerSynced}
